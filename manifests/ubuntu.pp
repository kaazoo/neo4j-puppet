...
class neo4j::ubuntu {
  ...
  exec {
    ...
    'bump the minimum heap size':
      ...
      unless      => '/bin/grep -q "^wrapper.java.initmemory" /etc/neo4j/neo4j-wrapper.conf',
    ...
    'bump the maximum heap size':
      ...
      require     => Package['neo4j'],
      unless      => '/bin/grep -q "^wrapper.java.maxmemory" /etc/neo4j/neo4j-wrapper.conf',
      notify      => Service['neo4j-service'];
  }
  ...
  file {
    ...
    'neo4j auth extension':
      path    => '/usr/share/neo4j-contrib/authentication-extension-2.0.0-M04-1.0-SNAPSHOT.jar',
      source  => 'puppet:///modules/neo4j/authentication-extension-2.0.0-M04-1.0-SNAPSHOT.jar',
      require => File['neo4j contrib dir'];

    'neo4j auth extension link':
      ensure  => 'link',
      path    =>  '/usr/share/neo4j/plugins/authentication-extension-2.0.0-M04-1.0-SNAPSHOT.jar',
      target  => '/usr/share/neo4j-contrib/authentication-extension-2.0.0-M04-1.0-SNAPSHOT.jar',
      require => File['neo4j auth extension'],
      notify  => Service['neo4j-service'];

    'neo4j apt config':
      path    => '/etc/apt/sources.list.d/neo4j.list',
      content => $neo4j::params::neo4j_debian_repository,
